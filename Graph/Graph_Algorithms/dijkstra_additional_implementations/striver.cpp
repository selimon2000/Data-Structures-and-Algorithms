...
class Solution {
    ...
    vector <int> dijkstra(int V, vector<vector<int>> adj[], int S) {
        // Create a set ds for storing the nodes as a pair {dist,node} where dist is the distance from source to the node.
        ...
        // Now, erase the minimum distance node first from the set and traverse for all its adjacent nodes.
        while(!st.empty()) {
            auto it = *(st.begin()); 
            int node = it.second; 
            int dis = it.first; 
