...
void permutations(vector<vector<int>>& res, vector<int>& arr, const int idx) {
    ...
    // Use a set to track elements that have already been used at this level of recursion
    unordered_set<int> used;

    // Permutations made by swapping each element, starting from index `idx`
    for (int i = idx; i < arr.size(); ++i) {
        // Skip if the current element has already been used at this level
        ...
        swap(arr[idx], arr[i]);          // Backtracking, required to return the array back to how it was before
    }
}
