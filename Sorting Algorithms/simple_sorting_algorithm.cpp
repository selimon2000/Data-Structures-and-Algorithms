...
// rather it keeps swapping values if they are smaller, rather than finding the smallest one, as done in selection sort
// This is inefficient
void simpleSortingFunction(int* my_array, int n) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
...
int main() {
    ...
    simpleSortingFunction(my_array, n);
